...
function loadTransactionHistory() {
  ...
  allTransactions = userTransactions.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
...
function displayTransactions(transactions) {
  ...
  const transactionHTML = transactions.map(transaction => {
    ...
    const amountClass = (transaction.type === 'topup' || transaction.type === 'receive') ? 'positive' : 'negative';
    const amountSign = (transaction.type === 'topup' || transaction.type === 'receive') ? '+' : '-';
    ...
    return `
      <div class="transaction-item" data-type="${transaction.type}">
        ...
        <div class="transaction-details">
          <div class="transaction-type">${getTransactionTitle(transaction.type)}</div>
...
function getTransactionTitle(type) {
  switch(type) {
    ...
    case 'send': return 'Transfer Keluar';
    case 'receive': return 'Transfer Masuk';
